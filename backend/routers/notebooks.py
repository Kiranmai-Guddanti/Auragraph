...
class NoteVersionCreateRequest(BaseModel):
    note: str = ""
    proficiency: Optional[str] = None
    reason: str = "mutation"
    max_versions: int = Field(default=10, ge=5, le=10)


@router.post("/api/notebooks/{nb_id}/versions")
async def create_note_version_endpoint(
    nb_id: str,
    req: NoteVersionCreateRequest,
    authorization: Optional[str] = Header(None),
):
    from agents.notebook_store import create_note_version
    user = get_current_user(authorization)
    _require_notebook_owner(nb_id, user)
    v = create_note_version(
        nb_id,
        req.note,
        req.proficiency,
        req.reason,
...
@router.get("/api/notebooks/{nb_id}/versions")
async def list_note_versions_endpoint(
    nb_id: str,
    limit: int = Query(default=10, ge=1, le=10),
    authorization: Optional[str] = Header(None),
):
    from agents.notebook_store import get_note_versions
    user = get_current_user(authorization)
    _require_notebook_owner(nb_id, user)
    return {"versions": get_note_versions(nb_id, limit)}


@router.post("/api/notebooks/{nb_id}/versions/{version_id}/restore")
async def restore_note_version_endpoint(
    nb_id: str,
    version_id: str,
    authorization: Optional[str] = Header(None),
):
    from agents.notebook_store import restore_note_version
    user = get_current_user(authorization)
    _require_notebook_owner(nb_id, user)
    nb = restore_note_version(nb_id, version_id)
    if not nb:
