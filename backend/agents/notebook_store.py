...
def create_note_version(
    ...
    """Store a note snapshot for version history and keep only newest N versions."""
    max_keep = max(5, min(10, int(max_versions or 10)))
    ...
    with _conn() as con:
        ...
        # Keep only the newest `max_keep` versions per notebook.
        con.execute(
            """
