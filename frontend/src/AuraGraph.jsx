        ...
        return (
            <figure style={{ margin: '14px 0' }}>
                <img
                    ...
                    alt={alt || 'Figure'}
                    loading="lazy"
                    style={{ width: '100%', maxHeight: 1200, objectFit: 'contain', borderRadius: 10, border: '1px solid #1e3a5f', background: '#020c1b' }}
                />
                {alt && (
            ...
            <div style={{ marginBottom: 28 }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                    {[
                        ...
                        <button key={p.label} onClick={() => setProficiency(p.label)} style={{ flex: 1, padding: '12px 8px', borderRadius: 8, cursor: 'pointer', border: '1px solid ' + (proficiency === p.label ? '#000' : '#1e293b'), background: proficiency === p.label ? '#000' : '#020c1b', color: proficiency === p.label ? '#fff' : '#475569', transition: 'all 0.15s', textAlign: 'center' }}>
                    ...
                    <div style={{ padding: "8px 16px", borderTop: border, display: "flex", gap: 10 }}>
                        {Object.entries(S).map(([k, v]) => (
                            <div key={k} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                ...
                                <span style={{ fontSize: 9, color: "#475569", textTransform: 'none' }}>{k}</span>
                            </div>
                        ))}
                    </div>
                    {/* Examiner panel */}
