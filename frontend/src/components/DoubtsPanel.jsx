                    ...
                    return (
                        <div key={d.id} id={'doubt-' + d.id}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6, marginBottom: 4 }}>
                                ...
                                {d.success && d.source && (
                                    <span style={{ fontSize: 8, fontWeight: 600, padding: '1px 5px', borderRadius: 6, background: d.source === 'azure' ? (isDark ? '#172554' : '#EFF6FF') : d.source === 'groq' ? (isDark ? '#052E16' : '#ECFDF5') : (isDark ? '#27272A' : '#F5F5F5'), color: d.source === 'azure' ? (isDark ? '#93C5FD' : '#1D4ED8') : d.source === 'groq' ? (isDark ? '#86EFAC' : '#065F46') : (isDark ? '#D4D4D8' : '#52525B'), border: `1px solid ${d.source === 'azure' ? (isDark ? '#1D4ED8' : '#BFDBFE') : d.source === 'groq' ? (isDark ? '#166534' : '#A7F3D0') : (isDark ? '#3F3F46' : '#D4D4D8')}` }}>{d.source.charAt(0).toUpperCase() + d.source.slice(1)}</span>
                                )}
