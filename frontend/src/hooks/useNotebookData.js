...
export function useNotebookData(id, deps = {}) {
    ...
    const saveMutationVersion = useCallback(async (snapshotNote, reason = 'mutation', maxVersions = 5) => {
    ...
    const getMutationVersions = useCallback(async (limit = 5) => {
        try {
            const res = await apiFetch(`${API}/api/notebooks/${id}/versions?limit=${Math.max(1, Math.min(5, limit))}`);
