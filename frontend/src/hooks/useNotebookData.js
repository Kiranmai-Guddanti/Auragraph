...
import { ls_getNotebook, ls_saveNote, ls_addNoteVersion, ls_getNoteVersions, ls_restoreNoteVersion } from '../localNotebooks';
...
export function useNotebookData(id, deps = {}) {
    ...
    const saveMutationVersion = useCallback(async (snapshotNote, reason = 'mutation', maxVersions = 10) => {
        const localVersion = ls_addNoteVersion(id, snapshotNote, prof, reason, maxVersions);
        try {
            const res = await apiFetch(`${API}/api/notebooks/${id}/versions`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ note: snapshotNote, proficiency: prof, reason, maxVersions }),
            });
            if (!res.ok) throw new Error('version save failed');
            const j = await res.json();
            return j.version || localVersion;
        } catch {
            return localVersion;
        }
    }, [id, prof]);

    const getMutationVersions = useCallback(async (limit = 10) => {
        try {
            const res = await apiFetch(`${API}/api/notebooks/${id}/versions?limit=${Math.max(1, Math.min(10, limit))}`);
            if (!res.ok) throw new Error('version list failed');
            const j = await res.json();
            return j.versions || [];
        } catch {
            return ls_getNoteVersions(id, limit);
        }
    }, [id]);

    const restoreMutationVersion = useCallback(async (versionId) => {
        try {
            const res = await apiFetch(`${API}/api/notebooks/${id}/versions/${versionId}/restore`, { method: 'POST' });
            if (!res.ok) throw new Error('restore failed');
            const nb = await res.json();
            setNote(nb.note || '');
            setProf(nb.proficiency || 'Practitioner');
            ls_saveNote(id, nb.note || '', nb.proficiency || 'Practitioner');
            return nb;
        } catch {
            const nb = ls_restoreNoteVersion(id, versionId);
            if (!nb) return null;
            setNote(nb.note || '');
            setProf(nb.proficiency || 'Practitioner');
            return nb;
